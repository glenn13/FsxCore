...
const Depreciation: React.FC<DepreciationProps> = ({isReadOnly}) => {
  ...
  const componentDepreciationDetailReducer = useSelector(
    (state: RootState) => state.componentDepreciationDetailReducer,
  );
  ...
  const handleFormikSubmit = () => {
    dispatch(
      setComponentDepreciationDetail(
  ...
  React.useEffect(() => {
    _straightLineEntity.straightLineVariable.purchaseCost =
      formik.values.componentPurchase?.totalAcquisitionAmount || 0;
    setStraightLineEntity(_straightLineEntity);
  }, [formik.values.componentPurchase?.totalAcquisitionAmount]);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <FsxTable data={componentDepreciationDetailReducer.current} columns={columns}>
        <FsxTableActions isReadOnly={isReadOnly}>
          <FsxTableAction
            label="Generate Details"
            onClick={handleGenerate}
            disabled={(formik.values.componentDepreciation?.depreciationMethodId || 0) <= 0}
          />
