...
import {getRegisterPerRecord} from '@app/services/asset/register/register.service';
import { useHistory } from 'react-router-dom';
import { AssetCategoryEnum } from '@app/helpers/asset/enum';

export interface AssetRegisterPerRecordProps {
  filter?: string;
}

const AssetRegisterPerRecord: React.VFC<AssetRegisterPerRecordProps> = ({filter}) => {
  const history = useHistory();
  const [data, setData] = useState<RegisterSummary[]>();
  ...
  useEffect(() => {
    if (counter === 0) {
      ...
      (async () => {
        if (filter) {
          await getRegisterPerRecord(filter)
...
export default AssetRegisterPerRecord;
