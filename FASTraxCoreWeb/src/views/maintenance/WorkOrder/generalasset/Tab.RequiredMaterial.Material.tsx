...
import {
  addWorkOrderGeneralAssetMaterial,
  emptyWorkOrderGeneralAssetMaterial,
  removeWorkOrderGeneralAssetMaterial,
  updateWorkOrderGeneralAssetMaterial,
...
const Material: React.FC<MaterialProps> = () => {
  ...
  const [selected, setSelected] = React.useState<WorkOrderGeneralAssetMaterial>();
  const [
    varWorkOrderGeneralAssetMaterial,
    varSetWorkOrderGeneralAssetMaterial,
  ] = React.useState<WorkOrderGeneralAssetMaterial>(emptyWorkOrderGeneralAssetMaterial());

  const workOrderGeneralAssetMaterialReducer = useSelector(
    (state: RootState) => state.workOrderGeneralAssetMaterialReducer,
  );

  const ids = React.useMemo(
    () => workOrderGeneralAssetMaterialReducer.map((wovmr: any) => wovmr.tempId),
    [workOrderGeneralAssetMaterialReducer],
  );

  const formikRef = React.useRef<FormikProps<WorkOrderGeneralAssetMaterial>>(null);

  const excelExportRef = useRef<any>(null);
  const exportToExcel = () => excelExportRef.current?.exportAsExcel();

  const handleAdd = () => {
    varSetWorkOrderGeneralAssetMaterial(emptyWorkOrderGeneralAssetMaterial(ids));
    setIsOpen(true);
  };
  const handleClose = () => {
    varSetWorkOrderGeneralAssetMaterial(emptyWorkOrderGeneralAssetMaterial(ids));
  ...
  const handleDelete = () => {
    if (!selected) return;
    dispatch(removeWorkOrderGeneralAssetMaterial(selected));
  ...
  const handleEdit = () => {
    if (!selected) return;
    varSetWorkOrderGeneralAssetMaterial(selected);
    setIsOpen(true);
  };

  const handleFormikSubmit = (value: WorkOrderGeneralAssetMaterial) => {
    const action =
  ...
  const handleInputChange = (
    ...
    key: keyof WorkOrderGeneralAssetMaterial,
  ) => {
    varSetWorkOrderGeneralAssetMaterial(currentState => ({
      ...currentState,
      [key]: val,
    }));
  };
  ...
  return (
    <>
      <FsxDrawer
        ...
        <div className="flex flex-1 flex-col w-full py-4 px-8">
          <Formik
            ...
            initialValues={varWorkOrderGeneralAssetMaterial}
      ...
      <FsxExcelExport
        fileName={`WorkOrder_GeneralAsset_RequiredMaterial_${moment().format('YYYYMMDDHHmm')}`}
        data={workOrderGeneralAssetMaterialReducer}
        ref={excelExportRef}
        columns={columns}>
        <FsxTable
          data={workOrderGeneralAssetMaterialReducer}
