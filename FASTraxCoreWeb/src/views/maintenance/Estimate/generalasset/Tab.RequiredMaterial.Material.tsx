...
import {
  addEstimateGeneralAssetMaterial,
  emptyEstimateGeneralAssetMaterial,
  removeEstimateGeneralAssetMaterial,
  updateEstimateGeneralAssetMaterial,
} from '@app/store/maintenance/estimate/generalAssetMaterial.reducers';
...
const Material: React.FC<MaterialProps> = () => {
  ...
  const [selected, setSelected] = React.useState<EstimateGeneralAssetMaterial>();
  const [
    varEstimateGeneralAssetMaterial,
    varSetEstimateGeneralAssetMaterial,
  ] = React.useState<EstimateGeneralAssetMaterial>(emptyEstimateGeneralAssetMaterial());

  const estimateGeneralAssetMaterialReducer = useSelector(
    (state: RootState) => state.estimateGeneralAssetMaterialReducer,
  );

  const ids = React.useMemo(
    () => estimateGeneralAssetMaterialReducer.map((wovmr: any) => wovmr.tempId),
    [estimateGeneralAssetMaterialReducer],
  );

  const formikRef = React.useRef<FormikProps<EstimateGeneralAssetMaterial>>(null);

  const excelExportRef = useRef<any>(null);
  const exportToExcel = () => excelExportRef.current?.exportAsExcel();

  const handleAdd = () => {
    varSetEstimateGeneralAssetMaterial(emptyEstimateGeneralAssetMaterial(ids));
    setIsOpen(true);
  };
  const handleClose = () => {
    varSetEstimateGeneralAssetMaterial(emptyEstimateGeneralAssetMaterial(ids));
  ...
  const handleDelete = () => {
    if (!selected) return;
    dispatch(removeEstimateGeneralAssetMaterial(selected));
  ...
  const handleEdit = () => {
    if (!selected) return;
    varSetEstimateGeneralAssetMaterial(selected);
    setIsOpen(true);
  };

  const handleFormikSubmit = (value: EstimateGeneralAssetMaterial) => {
    const action =
  ...
  const handleInputChange = (
    ...
    key: keyof EstimateGeneralAssetMaterial,
  ) => {
    varSetEstimateGeneralAssetMaterial(currentState => ({
      ...currentState,
      [key]: val,
    }));
  };
  ...
  return (
    <div className="p-2">
      <FsxDrawer
        ...
        <div className="flex flex-1 flex-col w-full py-4 px-8">
          <Formik
            ...
            initialValues={varEstimateGeneralAssetMaterial}
      ...
      <FsxExcelExport
        fileName={`WorkEstimate_GeneralAsset_RequiredMaterial_${moment().format('YYYYMMDDHHmm')}`}
        data={estimateGeneralAssetMaterialReducer}
        ref={excelExportRef}
        columns={columns}>
        <FsxTable
          data={estimateGeneralAssetMaterialReducer}
