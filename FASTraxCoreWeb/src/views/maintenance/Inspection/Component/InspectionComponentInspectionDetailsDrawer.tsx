...
import {useComponentMaintenanceInspectionAreas} from '@app/services/maintenance/standardentries/maintenanceInspectionArea';
...
const InspectionComponentInspectionDetailsDrawer = (props: {callBack?: () => void}) => {
  let {id} = useParams<any>();
  const {values, setFieldValue} = useFormikContext<InspectionComponent>();

  const {data: maintenanceInspectionAreas} = useComponentMaintenanceInspectionAreas();
      ...
      setGridData(
        _.filter(
          ...
          item =>
            !_.includes(
              _.flatMap(
                values.inspectionComponentInspectionDetails,
                x => x.maintenanceComponentInspectionAreaId,
              ),
              item.id,
            ),
        ).map(dataGrid => ({...dataGrid, selected: false})),
      );
  }, [maintenanceInspectionAreas, values.inspectionComponentInspectionDetails]);
  ...
  const handleButton = () => {
    const selectedData = _.filter(gridData, 'selected').map(data => {
      return {
        inspectionComponentId: id ? parseInt(id) : 0,
        actionTake: '',
        details: data.description,
        maintenanceComponentInspectionAreaId: data.id,
        passed: false,
        remarks: '',
      };
    });
    ...
    setFieldValue(
      ...
      produce(values.inspectionComponentInspectionDetails, draftData => {
...
export default InspectionComponentInspectionDetailsDrawer;
