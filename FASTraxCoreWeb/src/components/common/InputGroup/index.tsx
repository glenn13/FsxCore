...
const InputStyled = styled(Input)<IInputProps>`
  ...
  outline: none;
  padding: 12px 16px 7px ${props => (props.icon ? '33' : '16')}px !important;
  line-height: 1.5;
  font-size: 10pt;
  width: 100%;
  font-weight: 300;
  ...
  render() {
    ...
    return (
      <InputWrapperStyled>
        ...
        {rest.icon && <IconStyled className={iconName} iconColor={iconColor} />}
        <div
          className={clx('flex flex-row w-full', {
            'btn-rounded': rounded,
            'btn-shadow': shadow,
          })}>
          <div
            className="h-12 flex items-center justify-center bg-gray-800 text-white rounded-l-md"
            style={{minWidth: '48px'}}>
            {inputGroupText}
          </div>
          <div className="flex flex-col w-full">
            ...
            {!this.state.changed && errors && (
              <span className="text-xs text-red-400">{errors[0]}</span>
            )}
          </div>
        </div>
        {type === 'password' && (
